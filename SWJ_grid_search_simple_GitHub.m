%% SWJ_GRID_SEARCH_SIMPLE_GITHUB.M
% =========================================================================
%  Square-wave jerk (SWJ) grid-search pipeline
%
%  Purpose
%  -------
%  This script detects right-eye saccades during attempted fixation and
%  applies a grid search of candidate SWJ operational definitions. It is a
%  simplified GitHub-facing version of the analysis code used for the
%  manuscript.
%
%  Workflow
%  --------
%    1. Read each exported eye-tracking CSV file.
%    2. Detect right-eye saccades and export Saccades_right_main.
%    3. Sweep candidate SWJ criteria across amplitude, intra-SWJ interval,
%       direction-reversal tolerance, and amplitude similarity.
%    4. Export one threshold-sweep CSV per file and one master CSV across
%       all files.
%
%  Output files
%  ------------
%    <filename>_Saccades_right_main.csv
%    <filename>_SWJ_threshold_sweep.csv
%    SWJ_threshold_sweep_all_files.csv
%
...
GRID.simTypeList    = "dissim";  % |A1 - A2| / |A1 + A2|
...
for fileIdx = 1:numel(fileList)
    ...
    % 1. Read the exported eye-tracking CSV file.
    ...
    % Export detected main saccades for quality control and reuse.
...
function details = detectSWJPairs(Saccades, params, validGaze, cfg)
% Detect SWJ pairs from Saccades_right_main.
% This GitHub version allows one intervening detected saccade by default.
%
% Pair definition:
...
%   - one intervening detected saccade is allowed
%
...
%   14 PairMeanVerticalComp
    ...
    while i < n
        ...
        pairFound = false;
        % Allow one intervening detected saccade between the two SWJ components.
        maxJ = min(n, i + 2);
...
% Calculate final summary metrics for a set of SWJ pairs.
    ...
    pairAmp = mean(details(:, 3:4), 2);

    metrics.MeanPairAmp_deg = mean(pairAmp);
    metrics.Total_Amp = sum(pairAmp);
    metrics.MeanISI_ms = mean(details(:, 10));
    metrics.MeanPairVerticalComp = mean(details(:, 14));
end
